...
public class FormsTreeView extends Composite implements SelectionHandler<TreeItem>,IFormChangeListener,IFormActionListener{
    ...
    public void addNewForm(){
        int id = ++nextFormId;
        if (FormUtil.getFormDisplayName() != null && (FormUtil.getFormDisplayName()).length() > 0) {
            addNewForm(FormUtil.getFormDisplayName(), FormUtil.getFormDisplayNameId(), id);
        }
        else {
		...
		/*if(FormUtil.rebuildBindings()){
			if(parentObj instanceof QuestionDef){
				...
				if(parentQuestionDef != null && parentQuestionDef.getDataType() == QuestionDef.QTN_TYPE_REPEAT)
					...
					//return parentQuestionDef.getRepeatQtnsDef().getQuestionsCount() + 1; TODO, test line above
			}
